# Hangman Game

## Why this exists

A text-based Hangman game built to practice stateful gameplay and user interaction in the terminal.

## What this project shows

It demonstrates string handling, arrays, game state tracking, and giving responsive feedback to the player.

## How to run (local)

Open this folder in your terminal and compile with GCC or your C compiler.

Single program example:
  gcc main.c -o app -mconsole
  ./app   (or .\app.exe on Windows)

For folders with multiple small programs:
  gcc file_name.c -o file_name.exe -mconsole
  .\file_name.exe
